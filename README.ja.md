...
- [x] 1-1～7-5 の通常海域向け所持艦隊提案（出典付き攻略編成を採用し、自動モードは完全検証済みルートを優先し、存在しない場合は設定警告付きの計算可能ルートを使用。通常装備スロットの完全充足、2-1 の空母代替編成と高速建造材ルート、KC3 現行の水戦／航空機／砲弾カテゴリ、2-5／5-5 のドラム缶分岐、検証済みの高速＋装備、補強増設、夜戦空母編成、4-5 複合対地編成、5-5 特殊砲撃の組み合わせ・配置・陣形案内、KC3 の完成装備ボーナスと目標別実効火力による再評価を含む）
