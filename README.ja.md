...
4. **[任務推薦](./docs/quest-recommendations.md)** — KC3 の流程表に隣接する戦略室の**任務推薦**ページでは任務名を日本語原文で表示し、ゲームの最新状態を手動同期できます。初期状態ですべて有効な第一～第七海域フィルターで出撃任務を絞り込み、開放中の反復任務と通常の単発任務を現在と後続の報酬から評価します。任務条件から受注済み任務最大 5 件の両立可能な演習・通常海域出撃を自動算出し、検証済みの遠征・工廠の同時進行にも対応します。出撃以外の任務は海域フィルターに関係なく上部に固定し、表示中の任務一覧、全条件、計画詳細は Markdown に出力できます。
