...
### v1.0.9 更新内容（v1.0.8 以降）

- 通常海域推薦で、重複と誤判していた Bahamut 編成 4 件を修正しました。3-3 北方海域警備の
  軽巡編成、3-4 高速空母洗地、現行のランダム開始を反映した 3-5 速吸／山汐丸編成、任務指定艦
  4 隻を固定する 5-3 クォータリー三川編成を追加し、審査済みカタログは全 162 編成、うち
  Bahamut 直接出典 34 編成になりました。

### v1.0.8 更新内容（v1.0.7 以降）

- 通常海域推薦に、行飛の Bahamut 画像ガイドから 1-4～6-5 の重複しない 30 編成と、
  検証済みの 4-1 KCWiki 代替編成 2 件を追加しました。出典は編成ごとに保持し、未モデル化の
  出撃条件には手動警告を残すため、古い画像編成に合わせて現行のルート・装備条件を緩めません。
  3-4 の空母レベリングルートも A-C-E-G-J-P に修正しました。
- 今後の攻略 URL と編成画像を取り込み、重複確認、現行海域条件の検証、情報を確実に読めない
  場合の質問を行うプロジェクト skill `kancolle-stage` を追加しました。
...
4. **[任務推薦](./docs/quest-recommendations.md)** — KC3 の流程表に隣接する戦略室の**任務推薦**ページでは任務名を日本語原文で表示し、ゲームの最新状態を手動同期できます。初期状態ですべて有効な第一～第七海域フィルターで出撃任務を絞り込み、開放中の反復任務と通常の単発任務を現在と後続の報酬から評価します。検証済みの同時進行判定は出撃・演習・遠征・工廠を対象とし、出撃以外の任務は海域フィルターに関係なく上部に固定します。表示中の任務一覧、全条件、計画詳細は Markdown に出力できます。
...
- [x] 1-1～7-5 の通常海域向け所持艦隊提案（出典付き攻略編成を採用し、戦略室では出典付き参考攻略テンプレートを選択し、空の自動ルート選択は表示しません。通常装備スロットの完全充足、2-1 の空母代替編成と高速建造材ルート、KC3 現行の水戦／航空機／砲弾カテゴリ、2-5／5-5 のドラム缶分岐、検証済みの高速＋装備、補強増設、夜戦空母編成、出典準拠の 3-5 上ルート・潜水艦・Nelson Touch・固定／ランダム下ルート編成、4-4 の艦隊全体での柔軟な制空配分と伊勢／日向改二の瑞雲立体攻撃、4-5 高速＋夜母・夜母小型艦・Nelson Touch・迂回編成と複合対地編成向けの制空優先かつ柔軟な空母装備、5-5 特殊砲撃の組み合わせ・配置・陣形案内、KC3 の完成装備ボーナスと目標別実効火力による再評価を含む）
- [x] 任務名を日本語原文で表示し、最新状態を手動同期。初期状態ですべて有効な第一～第七海域フィルターで出撃任務を絞り込み、出撃・演習・遠征・工廠の検証済み同時進行を判定し、出撃以外の任務を海域フィルターに関係なく上部へ固定
