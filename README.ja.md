...
### v1.0.16 更新内容（v1.0.15 以降）

- 艦隊推薦・遠征計画・資源統計を個別のバックグラウンドワーカーで処理し、ある処理のタイムアウトが
  他の処理を失敗させないようにしました。
- アカウント再同期を開いている戦略室ウィンドウへ通知し、古い結果による上書きを防ぎ、同条件の計算を共有します。
- プロキシの起動・停止・再試行と更新確認のライフサイクルを改善しました。画像処理と ZIP 展開は専用ワーカーへ
  移し、メインプロセスの停止時間を減らします。

### v1.0.15 更新内容（v1.0.14 以降）

- 艦隊推薦で空母装備、対潜担当、特殊装備の搭載艦を比較し、代替案も KC3 の検証対象に残します。
  装備不足時は通常スロットの空きを許容します。
- 制空値の重複計算を減らし、選抜された候補のみ装備状態を構築することで、探索上限と必須条件を
  維持したまま装備計画を高速化しました。
- 同時リクエストはアカウント同期を共有し、速力・先制対潜の試算と分割処理を効率化しました。
  再同期時は他タブの古いデータも無効化し、処理段階ごとの診断を記録します。
...
- [x] [戦略室のウィンドウ間でアカウントスナップショットを共有し、艦隊推薦・遠征計画・資源統計を個別のバックグラウンドワーカーで処理](./docs/shell-runtime-architecture.md)
