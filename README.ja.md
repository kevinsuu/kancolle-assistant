...
8. **[日別改修フィルター](./docs/daily-improvement-filter.md)** — KC3 の日別改修一覧で、KC3 標準の改修可能装備フィルターを初回に適用し、全件表示へ戻す切り替えも維持します。
...
- [x] 1-1～7-5 の通常海域向け所持艦隊提案（出典付き攻略編成を採用し、自動選択はボス固定ルートに限定。検証済みの高速＋装備、補強増設、夜戦空母編成、4-5 三式弾編成を含む）
- [x] 艦隊割り当てと重み設定を含む遠征推薦
- [x] KC3 の固定期間向け資源収支サマリー
- [x] KC3 戦略室で順序を維持する最大 5 件のピン留めリンク
- [x] KC3 の日別改修一覧で改修可能装備フィルターを初期状態で有効化
