...
### v1.0.2 更新内容（v1.0.1 以降）

- Release tag による一般 CI の重複起動をなくし、推薦コアテストを明示的なクロスプラットフォーム対応パスに変更して、Windows の release 検証ですべてのテストファイルを検出できるようにしました。

### v1.0.1 更新内容（v1.0.0 以降）

- ウィンドウの縦横比を固定せず、リサイズ時にゲームキャンバスを再調整します。
- 所有装備による高速＋・夜戦空母編成を検証し、増速後の戦力で順位付けし、雷巡に適切な装備を割り当てます。
- アカウントと装備の索引を再利用し、必要な合法編成が揃った時点で探索を終了し、戦略室全体の再走査を削減します。
- 通常海域と遠征の判定には、EO ルート更新、低速艦隊ルート、水上戦闘機配備、バケツ報酬の優先化を反映しました。
...
4. **[遠征推薦](./docs/expedition-resource-planner.md)** — 戦略室の独立した**遠征推薦**ページが現在の資源を表示し、調整可能な4資源・バケツの重み、選択した遠征、成功・大発設定、第 2～4 艦隊の状態から最適な組み合わせを 1 件提案します。既存の Expedition Scorer は変更せず、遠征への自動派遣も行いません。
...
- [x] 1-1～7-5 の通常海域向け所持艦隊提案（出典付き攻略編成を採用し、自動選択はボス固定ルートに限定。検証済みの高速＋装備、補強増設、夜戦空母編成を含む）
