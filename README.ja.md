...
4. **[任務推薦](./docs/quest-recommendations.md)** — KC3 の流程表に隣接する戦略室の**任務推薦**ページでは任務名を日本語原文で表示し、ゲームの最新状態を手動同期できます。編成、出撃、演習、遠征、工廠、改装、その他の任務種別を複数選択でき、初期状態ですべて有効な第一～第七海域フィルターで出撃任務を絞り込み、開放中の反復任務と通常の単発任務を現在と後続の報酬から評価します。任務条件から受注済み任務最大 5 件の両立可能な演習・通常海域出撃を自動算出し、検証済みの遠征・工廠の同時進行にも対応します。出撃以外の任務は海域フィルターに関係なく上部に固定し、フィルターと並び順はローカルに保存され、ページ移動後やゲーム再起動後も復元されます。表示中の任務一覧、全条件、計画詳細は Markdown に出力できます。
...
- [x] 任務名を日本語原文で表示し、最新状態を手動同期。任務種別を複数選択でき、初期状態ですべて有効な第一～第七海域フィルターで出撃任務を絞り込み、出撃・演習・遠征・工廠の検証済み同時進行を判定し、出撃以外の任務を海域フィルターに関係なく上部へ固定。フィルターと並び順をローカルに保存
