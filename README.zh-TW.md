...
### v1.0.4 更新重點（相較 v1.0.3）

- 4-5 自動推薦現在會為相容艦娘保留三件不重複的帳號持有三式彈系裝備；若無法成立，也會顯示明確原因。
- KC3 每日改修頁面現在預設啟用原生的可改修裝備篩選；使用者仍可切回完整清單。
...
- [x] 1-1 至 7-5 普通海域的帳號艦隊推薦，採用附來源的攻略網驗證編成骨架，自動模式優先完整驗證路線，沒有時改用帶明確設定警告的可計算路線，並包含完整一般裝備槽、2-1 空母替代編成與高速建造材路線、KC3 現行水戰／航空機／砲彈分類、2-5／5-5 運輸桶分歧、高速＋裝備、補強增設欄位、夜戰空母配置、4-5 混合對陸配置、5-5 特殊砲擊配對、站位與陣形提示，以及 KC3 完整配裝加成與目標別有效火力重排
- [x] 含艦隊配對與權重設定的遠征推薦
- [x] KC3 固定近期區間資源收支摘要
