...
2. **[自適應遊戲畫面](./docs/display-auto-fit.md)** — 啟動遊戲時依 KC3 實際內容調整面板寬度，再以螢幕剩餘空間自動調整視窗與艦これ畫布；之後仍可手動調整。
3. **[普通海域艦隊推薦](./docs/fleet-recommender.md)** — 在 KC3 戰略室依目前帳號持有的艦娘與裝備，為 1-1 至 7-5（含 5-6）提供最多三組建議；不會自動變更遊戲狀態。
4. **[遠征與資源目標推薦](./docs/expedition-resource-planner.md)** — 戰略室獨立的**遠征推薦**頁面會依目前資源、目標、預設勾選的水桶收益、已勾選遠征與第 2～4 艦隊狀態，推薦一組最佳配對；不會改動原有遠征評分頁面，也不會自動派遣。
...
- [x] 依目前螢幕自動調整一次艦これ遊戲畫布
- [x] 1-1 至 7-5 普通海域的帳號艦隊推薦
- [x] 含艦隊配對的遠征資源目標規劃
- [x] KC3 固定近期區間資源收支摘要
