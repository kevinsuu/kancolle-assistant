...
8. **[每日改修篩選](./docs/daily-improvement-filter.md)** — KC3 每日改修頁面預設套用 KC3 原生的可改修裝備篩選一次，並保留切換按鈕供使用者查看完整清單。
...
- [x] 1-1 至 7-5 普通海域的帳號艦隊推薦，採用附來源的攻略網驗證編成骨架，自動模式只選固定進王路線，並包含已驗證的高速＋裝備、補強增設欄位、夜戰空母配置與 4-5 三式彈配置
- [x] 含艦隊配對與權重設定的遠征推薦
- [x] KC3 固定近期區間資源收支摘要
- [x] KC3 戰略室最多五個固定排序的釘選連結
- [x] KC3 每日改修頁面預設啟用可改修裝備篩選
