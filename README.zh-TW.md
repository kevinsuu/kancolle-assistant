...
4. **[任務推薦](./docs/quest-recommendations.md)** — 戰略室中與流程表相鄰的**任務推薦**頁面會以日文原文顯示任務標題，可手動同步遊戲最新狀態，以預設全選的第一章至第七章複選器篩選出擊任務，並依目前與後續獎勵排列所有已開放重複任務與一般單次任務。系統會從任務條件自動求出最多五個已接任務的相容演習與普通海域出擊共解，並保留經驗證的遠征及工廠共解；非出擊任務不受章節篩選影響並固定置頂。畫面上的任務清單、完整條件與規劃細節可匯出成 Markdown。
...
- [x] 以日文原文顯示任務標題、可手動同步最新狀態，以預設全選的第一章至第七章複選器篩選出擊任務，為出擊、演習、遠征與工廠提供經驗證的共解判定，並將不受章節篩選影響的非出擊任務固定置頂
