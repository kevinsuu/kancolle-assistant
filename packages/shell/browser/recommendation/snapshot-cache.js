...
export const createSnapshotCache = ({ logger = () => {}, notify = () => {}, maxResults = 128 }) => {
  ...
  const read = (load, force = false) => {
    ...
    if (!force && pending) {
      pending.consumerCount++
      return pending.promise
    }
    if (!force && snapshot) return Promise.resolve(snapshot)
    const version = ++generation
    const startedAt = Date.now()
    const capture = { consumerCount: 1, promise: null }
    const logCapture = (value, error) => {
      logger('recommendation.account-snapshot-request-completed', {
        generation: version,
        consumerCount: capture.consumerCount,
        outcome:
          disposed || version !== generation
            ? 'superseded'
            : error || value?.status === 'error'
              ? 'failed'
              : 'success',
        shipCount: value?.ships?.length ?? 0,
        equipmentCount: value?.equipment?.length ?? 0,
        reasonCodes: error ? ['SNAPSHOT_READ_FAILED'] : value?.error ? [value.error.code] : [],
