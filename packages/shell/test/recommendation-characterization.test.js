...
test('KC3 expedition planner preserves pairing, costs, modifiers, and tie-breakers', async () => {
  const normal = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld(baseRequest),
  )
  ...
  const modified = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld({
      ...baseRequest,
      fleetCount: 1,
      candidateIds: [1],
  ...
  const buckets = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld({ ...baseRequest, fleetCount: 1, considerBuckets: true }),
  )
...
test('KC3 expedition planner preserves no-solution outcomes', async () => {
  const targetReached = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld({
      ...baseRequest,
  ...
  const tooManyFleets = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld({ ...baseRequest, fleetCount: 4 }),
  )
  assert.equal(tooManyFleets.status, 'no-solution')
  assert.equal(tooManyFleets.reasonCode, 'INSUFFICIENT_FLEETS')

  const tooFewCandidates = await withRuntime(createExpeditionWindow(), () =>
    kc3ExpeditionPlannerMainWorld({ ...baseRequest, fleetCount: 2, candidateIds: [1] }),
  )
...
test('KC3 resource ledger preserves ranges, categories, and inventory carry-forward', async () => {
  const today = await withRuntime(createLedgerWindow(), () =>
    kc3ResourceLedgerMainWorld({ range: 'today' }),
  )
  ...
  const yesterday = await withRuntime(createLedgerWindow(), () =>
    kc3ResourceLedgerMainWorld({ range: 'yesterday' }),
  )
  ...
  const rolling = await withRuntime(createLedgerWindow(), () =>
    kc3ResourceLedgerMainWorld({ range: 'rolling24' }),
  )
