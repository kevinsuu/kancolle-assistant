...
4. **[任务推荐](./docs/quest-recommendations.md)** — 战略室中与流程表相邻的**任务推荐**页面会以日文原文显示任务标题，可手动同步游戏最新状态，通过默认全选的第一章至第七章多选器筛选出击任务，并根据当前与后续奖励排列所有已开放重复任务与普通单次任务。经验证的同一动作共解涵盖出击、演习、远征与工厂；非出击任务不受章节筛选影响并固定置顶。画面中的任务清单、完整条件与规划细节可导出为 Markdown。
