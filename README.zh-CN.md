...
- [x] 1-1 至 7-5 普通海域的账号舰队推荐，采用附来源的攻略网验证编成骨架，自动模式优先完整验证路线，没有时改用带明确设置警告的可计算路线，并包含完整普通装备栏位、2-1 航母替代编成与高速建造材路线、KC3 当前水战／航空机／炮弹分类、2-5／5-5 运输桶分歧、高速＋装备、补强增设栏位、夜战航母配置、4-5 混合对陆配置、5-5 特殊炮击配对、站位与阵形提示，以及 KC3 完整配装加成与目标别有效火力重排
- [x] 包含舰队配对与权重设置的远征推荐
- [x] KC3 固定近期时间段资源收支摘要
