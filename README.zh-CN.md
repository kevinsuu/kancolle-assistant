...
4. **[任务推荐](./docs/quest-recommendations.md)** — 战略室中与流程表相邻的**任务推荐**页面会以日文原文显示任务标题，可手动同步游戏最新状态，按编成、出击、演习、远征、工厂、改装或其他分类多选任务，通过默认全选的第一章至第七章多选器筛选出击任务，并根据当前与后续奖励排列所有已开放重复任务与普通单次任务。系统会根据任务条件自动求出最多五个已接任务的兼容演习与普通海域出击共解，并保留经验证的远征及工厂共解；非出击任务不受章节筛选影响并固定置顶。筛选与排序设置会保存在本地，切换页面或重启游戏后自动还原。画面中的任务清单、完整条件与规划细节可导出为 Markdown。
...
- [x] 以日文原文显示任务标题、可手动同步最新状态，可多选任务类型并通过默认全选的第一章至第七章多选器筛选出击任务，为出击、演习、远征与工厂提供经验证的共解判定，将不受章节筛选影响的非出击任务固定置顶，并在本地保存筛选与排序设置
