...
2. **[自适应游戏画面](./docs/display-auto-fit.md)** — 游戏启动时根据 KC3 实际内容调整面板宽度，再以屏幕剩余空间自动调整窗口和舰娘画布；之后仍可手动调整。
3. **[普通海域舰队推荐](./docs/fleet-recommender.md)** — 在 KC3 战略室根据当前账号拥有的舰娘和装备，为 1-1 至 7-5（含 5-6）提供最多三套建议；不会自动更改游戏状态。
4. **[远征与资源目标推荐](./docs/expedition-resource-planner.md)** — 战略室中独立的**远征推荐**页面会根据当前资源、目标、默认勾选的水桶收益、已勾选远征及第 2～4 舰队状态，推荐一套最佳配对；不会修改原有远征评分页面，也不会自动派遣。
...
- [x] 根据当前屏幕自动调整一次舰これ游戏画布
- [x] 1-1 至 7-5 普通海域的账号舰队推荐
- [x] 包含舰队配对的远征资源目标规划
- [x] KC3 固定近期时间段资源收支摘要
