...
### v1.0.13 更新重点（相比 v1.0.12）

- 指定路线的舰队推荐现在会将有效的当前舰队与有界的合成替代方案一并比较；在保留当前装备候选的
  同时，也能让更强的后备舰优先入选。
- 普通海域推荐新增两套有来源的ぜかまし 5-4“三一驱”路线，并加强共用三川任务舰队与
  33 式索敌条件；审查后的目录扩充至 167 套配置。

### v1.0.12 更新重点（相比 v1.0.11）

- 任务推荐新增可多选的任务类型筛选、建议优先级排序，并在切换页面或重启程序后保留筛选与排序设置。
- 任务共解会保留彼此重叠的替代方案，并从任务条件推导可共用的工厂废弃动作，同时维持最佳主要方案。
- 普通海域推荐新增两套有来源、无航母的 3-1 C-F-G 日英美年度任务编成，并使用标准舰名匹配，
  不受 KC3 显示语言影响。

### v1.0.11 更新重点（相比 v1.0.10）

- 任务推荐现在会根据各任务的行动、海域与编成条件，自动求出可兼容的演习及普通海域出击共解，
  最多合并五个已接任务；相互冲突的舰队条件仍会分开。
- 任务推荐的控件、标注与任务卡现在至少使用 12px 文字，与 KC3 导航栏一致并提升可读性。

### v1.0.10 更新重点（相比 v1.0.9）

- 战略室新增任务推荐：显示日文原文任务名称、手动同步游戏最新状态、按第一章至第七章与奖励
  筛选，并将后续奖励纳入排序；同时提供经验证的同一动作共解流程与画面计划 Markdown 导出。
- 舰队推荐加强受限候选搜索、当前舰队与 33 式索敌评估、建议制空值处理，以及有来源的
  1-6、2-1、2-5、4-4、5-5 配置，包括伊势／日向改二瑞云立体攻击提示。
- 远征规划设置会保存在本机，切换战略室页面或重启程序后自动恢复；无效存储数据会安全回退，
  并留下有界诊断。

### v1.0.9 更新重点（相比 v1.0.8）

- 普通海域推荐修正四套此前误判为重复的巴哈配置：3-3 北方海域警备轻巡编成、3-4
  高速航母洗地、按当前随机起点建模的 3-5 速吸／山汐丸编成，以及锁定四艘任务指定舰的
  5-3 季常三川编成。审查后的目录现包含 162 套配置，其中 34 套直接来自巴哈图文攻略。
...
4. **[任务推荐](./docs/quest-recommendations.md)** — 战略室中与流程表相邻的**任务推荐**页面会以日文原文显示任务标题，可手动同步游戏最新状态，按编成、出击、演习、远征、工厂、改装或其他分类多选任务，通过默认全选的第一章至第七章多选器筛选出击任务，并根据当前与后续奖励排列所有已开放重复任务、普通单次任务及当前可用的期间限定任务。期间限定任务会归入**其他**，KC3 未提供最终期限时也会明确标记。系统会根据任务条件自动求出最多五个已接任务的兼容演习与普通海域出击共解，并保留经验证的远征及工厂共解；非出击任务不受章节筛选影响并固定置顶。筛选与排序设置会保存在本地，切换页面或重启游戏后自动还原。画面中的任务清单、完整条件与规划细节可导出为 Markdown。
5. **[远征推荐](./docs/expedition-resource-planner.md)** — 战略室中独立的**远征推荐**页面会显示当前资源，并根据归一化每小时效率后的四项资源与水桶权重、已勾选远征、成功与大发设置及第 2～4 舰队状态，推荐一套最佳配对。规划设置会保存在本机，切换页面或重启游戏后自动恢复；不会修改原有远征评分页面，也不会自动派遣。
6. **[资源中心与收支摘要](./docs/resource-ledger-summary.md)** — 新增 KC3 战略室**资源中心**仪表板，可按今天、昨天或最近 24 小时查看当前资源、获取、消耗、净变化、每小时收支、来源分类与消耗品。
7. **[KC3 开发者工具集成](./docs/kc3-devtools.md)** — 打开游戏开发者工具时，优先排列并选中 KC3 的 `KanColle` 面板，减少重复手动切换。
8. **[战略室置顶链接](./docs/strategy-room-recent-tabs.md)** — 最多可将五个战略室标签页置顶到 `常用链接`；普通导航不会改变顺序，第六个置顶项会替换最下方链接。
9. **[每日改修筛选](./docs/daily-improvement-filter.md)** — KC3 每日改修页面默认应用一次原生的可改修装备筛选，并新增只包含当天可改修装备类别的横向筛选；原生切换按钮仍可查看完整列表。
...
- [x] 1-1 至 7-5 普通海域的账号舰队推荐，采用附来源的攻略网验证编成骨架，战略室需选择附来源的参考攻略配置、不再提供空白自动路线，并包含完整普通装备栏位、2-1 航母替代编成与高速建造材路线、KC3 当前水战／航空机／炮弹分类、2-5／5-5 运输桶分歧、高速＋装备、补强增设栏位、夜战航母配置、与来源一致的 3-5 上路、潜艇、Nelson Touch、固定与随机下路配置、4-4 全舰队弹性制空与伊势／日向改二瑞云立体攻击、4-5 高速＋夜母、夜母小船、Nelson Touch 与绕路配置及混合对陆配置的制空优先弹性航母配装、5-5 特殊炮击配对、站位与阵形提示，以及 KC3 完整配装加成与目标别有效火力重排
- [x] 以日文原文显示任务标题、可手动同步最新状态，可多选任务类型并通过默认全选的第一章至第七章多选器筛选出击任务，为出击、演习、远征与工厂提供经验证的共解判定，将不受章节筛选影响的非出击任务固定置顶；当前可用的期间限定任务归入“其他”并标记最终期限未知，且在本地保存筛选与排序设置
- [x] 将画面当前显示的任务、筛选、完成条件、奖励、期限、后续任务与建议共解流程导出为 Markdown
- [x] 包含舰队配对与权重设置的远征推荐
- [x] KC3 固定近期时间段资源收支摘要
